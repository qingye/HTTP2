...
public enum StreamState {
     ...
     *
     * <ul>
     * <li>Sending or receiving a HEADERS frame causes the stream to
     * become "open".  The stream identifier is selected as described
     * in Section 5.1.1.  The same HEADERS frame can also cause a
     * stream to immediately become "half-closed".</li>
     *
     * <li>Sending a PUSH_PROMISE frame on another stream reserves the
     * idle stream that is identified for later use.  The stream state
     * for the reserved stream transitions to "reserved (local)".</li>
     *
     * <li>Receiving a PUSH_PROMISE frame on another stream reserves an
     * idle stream that is identified for later use.  The stream state
     ...
     * <ul>
     * <li>The endpoint can send a HEADERS frame.  This causes the stream
     * to open in a "half-closed (remote)" state.</li>
     *
     * <li>Either endpoint can send a RST_STREAM frame to cause the stream
     ...
     * In this state, only the following transitions are possible:
     * <ul>
     * <li>Receiving a HEADERS frame causes the stream to transition to
