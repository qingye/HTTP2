 ...
 * though it cannot be sent between consecutive frames that comprise a single header block (Section 4.3).
 * Note that this frame could arrive after processing or frame sending has completed,
 * which would cause it to have no effect on the identified stream.
 * For a stream that is in the "half-closed (remote)" or "closed" state,
 * this frame can only affect processing of the identified stream and its dependent streams; it does not affect frame transmission on that stream.
 * <p>
 * The PRIORITY frame can be sent for a stream in the "idle" or "closed" state.
 * This allows for the reprioritization of a group of dependent streams by altering the priority of an unused or closed parent stream.
 * <p>
