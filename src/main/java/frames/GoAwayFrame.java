 ...
 * To deal with this case, the GOAWAY contains the stream identifier of the last peer-initiated stream that was or
 * might be processed on the sending endpoint in this connection. For instance, if the server sends a GOAWAY frame,
 * the identified stream is the highest-numbered stream initiated by the client.
 * <p>
 * Once sent, the sender will ignore frames sent on streams initiated by the receiver if the stream has an identifier
 * higher than the included last stream identifier. Receivers of a GOAWAY frame MUST NOT open additional streams on
 * the connection, although a new connection can be established for new streams.
 * <p>
 * If the receiver of the GOAWAY has sent data on streams with a higher stream identifier than what is indicated in
 ...
 * connection.
 * <p>
 * Endpoints SHOULD always send a GOAWAY frame before closing a connection so that the remote peer can know whether
 * a stream has been partially processed or not. For example, if an HTTP client sends a POST at the same time that a
 ...
 * <p>
 * The GOAWAY frame applies to the connection, not a specific stream. An endpoint MUST treat a GOAWAY frame with a
 * stream identifier other than 0x0 as a connection error (Section 5.4.1) of type PROTOCOL_ERROR.
 * <p>
 ...
 * <p>
 * Note: In this context, "processed" means that some data from the stream was passed to some higher layer of
 * software that might have taken some action as a result.
 * <p>
 * If a connection terminates without a GOAWAY frame, the last stream identifier is effectively the highest possible
 * stream identifier.
 * <p>
 * On streams with lower- or equal-numbered identifiers that were not closed completely prior to the connection being
 * closed, reattempting requests, transactions, or any protocol activity is not possible, with the exception of
 * idempotent actions like HTTP GET, PUT, or DELETE. Any protocol activity that uses higher-numbered streams can be
 ...
 * The sender of a GOAWAY frame might gracefully shut down a connection by sending a GOAWAY frame, maintaining the
 ...
 * <p>
 * A client that is unable to retry requests loses all requests that are in flight when the server closes the
 * connection. This is especially true for intermediaries that might not be serving clients using HTTP/2.
 * A server that is attempting to gracefully shut down a connection SHOULD send an initial GOAWAY frame with the
 * last stream identifier set to 231-1 and a NO_ERROR code. This signals to the client that a shutdown is imminent
 * and that initiating further requests is prohibited. After allowing time for any in-flight stream creation
 * (at least one round-trip time), the server can send another GOAWAY frame with an updated last stream identifier.
 * This ensures that a connection can be cleanly shut down without losing requests.
 * <p>
 * After sending a GOAWAY frame, the sender can discard frames for streams initiated by the receiver with identifiers
 * higher than the identified last stream. However, any frames that alter connection state cannot be completely
 * ignored. For instance, HEADERS, PUSH_PROMISE, and CONTINUATION frames MUST be minimally processed to ensure the
 * state maintained for header compression is consistent (see Section 4.3); similarly, DATA frames MUST be counted
 * toward the connection flow-control window. Failure to process these frames can cause flow control or header
 ...
 * <p>
 * Endpoints MAY append opaque data to the payload of any GOAWAY frame. Additional debug data is intended for
 * diagnostic purposes only and carries no semantic value. Debug information could contain security- or
 * privacy-sensitive data. Logged or otherwise persistently stored debug data MUST have adequate safeguards
