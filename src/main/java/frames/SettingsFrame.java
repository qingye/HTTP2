 ...
 * which are used by the receiving peer. Different values for the same parameter can be advertised by each peer.
 * For example, a client might set a high initial flow-control window,
 * whereas a server might set a lower value to conserve resources.
 * <p>
 * A SETTINGS frame MUST be sent by both endpoints at the start of a connection and MAY be sent at any other time
 * by either endpoint over the lifetime of the connection.
 ...
 * Parameters are processed in the order in which they appear,
 * and a receiver of a SETTINGS frame does not need to maintain any state other than the current value of its parameters.
 * Therefore, the value of a SETTINGS parameter is the last value that is seen by a receiver.
 * <p>
 * SETTINGS parameters are acknowledged by the receiving peer.
 * To enable this, the SETTINGS frame defines the following flag:
 * <p>
 * ACK (0x1):
 ...
 * connection error (Section 5.4.1) of type FRAME_SIZE_ERROR. For more information,
 ...
 * <p>
 * The SETTINGS frame affects connection state. A badly formed or incomplete SETTINGS frame MUST be treated as a
