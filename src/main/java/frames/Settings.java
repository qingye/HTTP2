...
public enum Settings {
     ...
     * signaling specific to the header compression format inside a header block (see [COMPRESSION]).
     ...
     * A zero value does prevent the creation of new streams; however, this can also happen for any limit
     * that is exhausted with active streams. Servers SHOULD only set a zero value for short durations;
     * if a server does not wish to accept requests, closing the connection is more appropriate.
     */
     ...
     * the maximum allowed frame size (2^24-1 or 16,777,215 octets), inclusive.
     ...
     * the sender is prepared to accept, in octets.
